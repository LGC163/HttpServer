...
void HttpRequest::setQueryParameters(const char* start, const char* end) {
    ...
    while ((pos = argumentStr.find('&', prev)) != std::string::npos) {
        std::string argument = argumentStr.substr(prev, pos - prev);
        std::string::size_type equalPos = argument.find('=');
        if (equalPos != std::string::npos) {
            std::string key = argument.substr(0, equalPos);
            std::string value = argument.substr(equalPos + 1);
